...
After proceeding, we end up with a sub-data set of 12 variables.
...
fig <- plot_ly(nan_percent, x = ~Colonne, y = ~Pourcentage_de_NaN, type = 'bar', text = ~Pourcentage_de_NaN,
               textposition = 'outside', texttemplate = '%{text:.2f}%')

# Mise à jour de l'axe des ordonnées pour afficher jusqu'à 100%
fig <- fig %>% layout(yaxis = list(range = c(0, 100), title = 'Percentage of NaN'),
...
The 'Production system' variable has a higher percentage of missing values, around 37.6%.
...
To do this, we are going to create a sub data set 'swiss_production_data' which includes all the Swiss production and sales regions.
...
# Supposons que vos données sont dans un fichier Excel nommé "data.xlsx"
file_path <- "../data/"
df <- read_excel(paste0(file_path, "data.xlsx"), sheet = 1)

# Faites une copie des données
clean_data <- df

# Supprimez les colonnes spécifiées
clean_data <- clean_data %>%
  select(-Devise, -Composants_de_coûts, -Type_de_données, -Source_de_données, 
         -Groupe_de_produits, -Commerce_extérieur, -Indicateur, -Marché, 
         -Propriétés_du_produit, -Mode_d_utilisation, -Unité, -Produit, 
         -Echelon_de_création_de_valeur, -Echelon_de_création_de_valeur_Détail)

# Convertir la colonne Date en format Date
clean_data$Date <- as.Date(clean_data$Date)

# Affichez les premières lignes des données nettoyées
head(clean_data)

# Créez une table tibble avec des descriptions des variables
variable_table <- tibble(
  Variable = c("Date", "Col1", "Col2", "Col3"), # Remplacez "Col1", "Col2", etc. par les noms réels des colonnes
  Description = c(
    "The date when the data was recorded, in a year-month-day format.",
    "Description for Col1", # Ajoutez les descriptions réelles de vos colonnes
    "Description for Col2",
    "Description for Col3"
  )
)
...
This data set explains the Milk price to producers over time.
Essential for Macro Analysis of the milk industry in Switzerland.
