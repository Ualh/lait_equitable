...
We loaded the dataset and renamed the columns to make it more readable. We also standardized the city names based on the provided mapping.
...
### Dataset Sales 2022
#### Wrangling and Cleaning
Following the same process as the 2023 dataset, we will load, clean, and standardize the 2022 sales data.
```{python}
...
#### Description
The 2022 dataset is not monthly like the 2023 dataset. It consists of multiple products and their total sales across various locations, with the following key columns:
```{r}
...
Indeed the 2022 dataset has a different structure from the 2023 data and is even more ligtht.
Have a look : 
```{r}
library(dplyr)
library(reactable)
