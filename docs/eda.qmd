...
swiss_production_data$Year <- as.numeric(format(swiss_production_data$Date, "%Y"))

# Calculer le prix moyen par système de production et par année
average_price <- swiss_production_data %>%
  group_by(`System.of.production`, Year) %>%
...
# Ajouter des titres et des légendes, et ajuster les paramètres de l'axe x
fig <- layout(fig,
              title = 'Average Production Price by System of Production and Year',
              xaxis = list(
                title = 'Year',
                tickmode = 'linear',
                dtick = 1,
              ...
              yaxis = list(title = 'Average Price in Centimes'),
