...
# Additive Decomposition of Milk Price in Switzerland
...
```{r}
# Code R pour générer la décomposition additive
fit_bio_add <- decompose(ts_bio, type = "additive")
autoplot(fit_bio_add) +
  labs(title = "Additive Decomposition of Organic Milk Prices", x = "Date", y = "")

fit_conventional_add <- decompose(ts_conventional, type = "additive")
autoplot(fit_conventional_add) +
  labs(title = "Additive Decomposition of Non-Organic Milk Prices", x = "Date", y = "")
```

```{r}
# Code R pour générer la décomposition multiplicative
fit_bio_mult <- decompose(ts_bio, type = "multiplicative")
autoplot(fit_bio_mult) +
  labs(title = "Multiplicative Decomposition of Organic Milk Prices", x = "Date", y = "")

fit_conventional_mult <- decompose(ts_conventional, type = "multiplicative")
autoplot(fit_conventional_mult) +
  labs(title = "Multiplicative Decomposition of Non-Organic Milk Prices", x = "Date", y = "")
```

There is no clear trend, it decreases and then increases with two peaks in 2008 and 2014.
There is a clear seasonal component due to weather conditions or agricultural cycles.
The residual component is relatively stable, but shows some peaks which could indicate anomalies or events not taken into account by the trend and seasonality model.

To determine whether our milk price data are better represented by an additive or multiplicative model, we will combine a visual analysis of the time series decomposition with quantitative methods, i.e. by comparing the AICs of the two models.

```{r}
# Ajuster les modèles ETS additifs et multiplicatifs
fit_bio_ets_add <- ets(ts_bio, model = "AAA")
fit_bio_ets_mult <- ets(ts_bio, model = "MAM")

fit_conventional_ets_add <- ets(ts_conventional, model = "AAA")
fit_conventional_ets_mult <- ets(ts_conventional, model = "MAM")

# Comparer les AICc
aic_bio_add <- fit_bio_ets_add$aicc
aic_bio_mult <- fit_bio_ets_mult$aicc

aic_conventional_add <- fit_conventional_ets_add$aicc
aic_conventional_mult <- fit_conventional_ets_mult$aicc

# Afficher les résultats
print(paste("AICc pour le modèle additif du lait bio : ", aic_bio_add))
print(paste("AICc pour le modèle multiplicatif du lait bio : ", aic_bio_mult))
print(paste("AICc pour le modèle additif du lait conventionnel : ", aic_conventional_add))
print(paste("AICc pour le modèle multiplicatif du lait conventionnel : ", aic_conventional_mult))
```
Multiplicative models have lower AIC values, indicating a better fit than additive models.
We will therefore use the multiplicative model in our subsequent analysis to best represent our forecasts.


# SARIMA model 
## ACF & PACF - including all system of production

We will then run a SARIMA model. Before applying this model, we need to check the stationarity of the data (and particularly the residuals) and determine the optimum values for the model parameters: p, d, q, P, D and Q.
To determine these parameters, we will use the graphs Autocorrelation Function ACF and Partial Autocorrelation Function PACF. These graphs will help us to identify the dependency structure in the data.
...
# Assurez-vous que les dates sont de type Date
swiss_decomposition <- swiss_decomposition %>%
  mutate(Date = as.Date(Date, format = "%Y-%m-%d")) %>%
  arrange(Date)

# Si vos données sont déjà mensuelles, il n'est pas nécessaire de les resampler. Vous pouvez les utiliser directement.
# Calculez la moyenne des prix par mois (facultatif si vous avez plusieurs enregistrements par mois)
data <- swiss_decomposition %>%
  group_by(Date) %>%
...
acf(data_ts, lag.max = 40, main = "", xlab = "Lags (Months)", col = "blue")
grid(nx = NULL, ny = NULL, col = "lightgray", lty = "dotted")

pacf(data_ts, lag.max = 40, main = "", xlab = "Lags (Months)", col = "blue")
grid(nx = NULL, ny = NULL, col = "lightgray", lty = "dotted")

# Différenciation saisonnière pour supprimer la saisonnalité annuelle
data_seasonal_diff <- diff(data_ts, lag = 12)

# Différenciation simple pour supprimer la tendance
data_diff <- diff(data_seasonal_diff)

# Tracer ACF et PACF pour la série différenciée
par(mfrow = c(2, 1), mar = c(4, 4, 2, 1), oma = c(0, 0, 2, 0))

acf(data_diff, lag.max = 40, main = "", xlab = "Lags (Months)", col = "blue")
grid(nx = NULL, ny = NULL, col = "lightgray", lty = "dotted")

pacf(data_diff, lag.max = 40, main = "", xlab = "Lags (Months)", col = "blue")
grid(nx = NULL, ny = NULL, col = "lightgray", lty = "dotted")
...
The blue zone represents the confidence interval around the zero line, set at 95%. Values outside this zone are considered statistically significant. If one of the values in our dataset falls outside this interval, this means that the autocorrelation is strong (i.e. significantly different from zero).

The y-axis represents the autocorrelation and partial autocorrelation coefficients.
 
The x-axis represents the lag. A lag of 1 means that autocorrelation is calculated between values at successive periods.

The first two graphs show the ACF and PACF of the original time series. 
The autocorrelation values are significant for several lags, indicating a trend or a seasonal component. The slow decay suggests that the data are not stationary. The presence of significant autocorrelation for many lags indicates a persistent trend in the data. This means that the series is non-stationary.
The PACF shows significant peaks at the first lags, followed by a rapid decrease. This suggests that the effect of autocorrelation diminishes after a few lags.
The first significant lags in the PACF suggest that a few autoregressive terms may be necessary to capture the structure in the data.

The last two graphs show the ACF and PACF of the differentiated series.
The differentiated series appears to be more stationary, as most of the autocorrelations are no longer significant. However, there may be some slight seasonality or residual effects.
For PACF, the first lags show some significant values, but most of the values quickly fall within the confidence interval.
The differentiated series shows a low-order autoregressive structure, suggesting that an ARIMA model with small parameters p and q may suffice.


## Overall SARIMA forecast 

Now that we have analysed these dependencies, we can set the parameters we need to apply the SARIMA model:

Differentiation (d) : Since the original series is not stationary and the differentiated series appears to be stationary, we will use d=1.
Autoregressive order (p) : The first lag in the PACF is significant, so p=1.
Moving average order (q): The ACF shows some significant lags, but they are not dominant, so q=1.

For seasonal components:
Seasonal differentiation (D): The data shows seasonality, so we use D=1.
Seasonal autoregressive order (P): The first lags in the seasonal PACF show slight significance, so P=1.
Seasonal moving average order (Q): Similar to q, we use Q=1.
Seasonal period (S) : As the data are monthly, we use S=12.
...
# Importer les données
swiss_decomposition <- read_csv('/Users/raimon/Desktop/Github_repo/lait_equitable/data/swiss_production_data.csv', show_col_types = FALSE)
...
ggplot() +
  ...
  labs(title = 'Overall SARIMA Forecast', x = 'Date', y = 'Price') +
  theme_minimal() +
...
## ACF & PACF for "Bio" & "Conventional"

We're interested in seeing how the two production systems develop over the coming months. We apply ACF & PACF to each system of production : "Bio" and "Conventional".
...
systemes_production <- c('Bio', 'Conventional')
...
for (systeme in systemes_production) {
  ...
  system_data <- swiss_decomposition %>%
    filter(`System of production` == systeme) %>%
    group_by(Date = floor_date(Date, "month")) %>%
    summarise(Price = mean(Price, na.rm = TRUE)) %>%
    ungroup()
  
  # Convertir en série temporelle
  system_ts <- ts(system_data$Price, start = c(year(min(system_data$Date)), month(min(system_data$Date))), frequency = 12)
  
  # Tracer ACF et PACF pour la série originale
  par(mfrow = c(2, 1), mar = c(4, 4, 2, 1), oma = c(0, 0, 4, 0))
  
  acf(system_ts, lag.max = 40, main = "", xlab = "Lags (Months)", col = "blue", cex.axis = 0.8, cex.lab = 0.8)
  grid(nx = NULL, ny = NULL, col = "lightgray", lty = "dotted")
  
  pacf(system_ts, lag.max = 40, main = "", xlab = "Lags (Months)", col = "blue", cex.axis = 0.8, cex.lab = 0.8)
  grid(nx = NULL, ny = NULL, col = "lightgray", lty = "dotted")
  
  # Ajouter un titre commun avec une taille réduite
  mtext(paste('Original Series -', systeme), outer = TRUE, cex = 1.2)
  
  # Différenciation saisonnière pour supprimer la saisonnalité annuelle
  system_seasonal_diff <- diff(system_ts, lag = 12)
  
  # Différenciation simple pour supprimer la tendance
  system_diff <- diff(system_seasonal_diff)
  
  # Tracer ACF et PACF pour la série différenciée
  par(mfrow = c(2, 1), mar = c(4, 4, 2, 1), oma = c(0, 0, 4, 0))
  
  acf(system_diff, lag.max = 40, main = "", xlab = "Lags (Months)", col = "blue", cex.axis = 0.8, cex.lab = 0.8)
  grid(nx = NULL, ny = NULL, col = "lightgray", lty = "dotted")
  
  pacf(system_diff, lag.max = 40, main = "", xlab = "Lags (Months)", col = "blue", cex.axis = 0.8, cex.lab = 0.8)
  grid(nx = NULL, ny = NULL, col = "lightgray", lty = "dotted")
  
  # Ajouter un titre commun avec une taille réduite
  mtext(paste('Differenciated Series -', systeme), outer = TRUE, cex = 1.2)
...
autoplot(forecast_bio_sarima) +
  labs(title = "Forecasted Prices of Organic Milk (SARIMA)", x = "Date", y = "Prix Moyen (centimes de CHF)") +
  theme_minimal()

autoplot(forecast_conventional_sarima) +
  labs(title = "Forecasted Prices of Non-Organic Milk (SARIMA)", x = "Date", y = "Prix Moyen (centimes de CHF)") +
  theme_minimal()
```
...
# Créer une variable exogène pour capturer l'effet de la crise de 2008
data_bio <- data_bio %>%
  mutate(Crisis2008 = ifelse(Date >= as.Date("2008-01-01") & Date <= as.Date("2010-01-01"), 1, 0))

data_conventional <- data_conventional %>%
  mutate(Crisis2008 = ifelse(Date >= as.Date("2008-01-01") & Date <= as.Date("2010-01-01"), 1, 0))
...
# Convertir la variable exogène en série temporelle
exog_bio <- ts(data_bio$Crisis2008, start = c(year(min(data_bio$Date)), month(min(data_bio$Date))), frequency = 12)
exog_conventional <- ts(data_conventional$Crisis2008, start = c(year(min(data_conventional$Date)), month(min(data_conventional$Date))), frequency = 12)

# Ajuster les modèles SARIMAX
fit_bio_sarimax <- auto.arima(ts_bio, xreg = exog_bio, seasonal = TRUE, stepwise = FALSE, approximation = FALSE)
fit_conventional_sarimax <- auto.arima(ts_conventional, xreg = exog_conventional, seasonal = TRUE, stepwise = FALSE, approximation = FALSE)

# Prévisions pour les 12 prochains mois avec la variable exogène
future_exog <- rep(0, 12)  # Supposons pas de crise pour les 12 prochains mois
forecast_bio_sarimax <- forecast(fit_bio_sarimax, xreg = future_exog, h = 12)
forecast_conventional_sarimax <- forecast(fit_conventional_sarimax, xreg = future_exog, h = 12)

# Tracer les prévisions
autoplot(forecast_bio_sarimax) +
  labs(title = "Forecasted Prices of Organic Milk (SARIMAX)", x = "Date", y = "Prix Moyen (centimes de CHF)") +
  theme_minimal()

autoplot(forecast_conventional_sarimax) +
  labs(title = "Forecasted Prices of Non-Organic Milk (SARIMAX)", x = "Date", y = "Prix Moyen (centimes de CHF)") +
  theme_minimal()
```
...
autoplot(forecast_bio_hw) +
  labs(title = "Forecasted Prices of Organic Milk (Holt-Winters Multiplicative)", x = "Date", y = "Prix Moyen (centimes de CHF)") +
  theme_minimal()

autoplot(forecast_conventional_hw) +
  labs(title = "Forecasted Prices of Non-Organic Milk (Holt-Winters Multiplicative)", x = "Date", y = "Prix Moyen (centimes de CHF)") +
  theme_minimal()
```
