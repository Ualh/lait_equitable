...
sales_data <- read_excel("Ventes annuelles.xlsx") %>%
  select(-`2022`)

# Read political party data from Excel
party_data <- read_excel("partisPolitiqueManor.xlsx")
...
plr_sales <- merge(sales_data, plr_data, by.x = "Location")
ps_sales <- merge(sales_data, ps_data, by.x = "Location")
udc_sales <- merge(sales_data, udc_data, by.x = "Location")
centre_sales <- merge(sales_data, centre_data, by.x = "Location")
verts_sales <- merge(sales_data, verts_data, by.x = "Location")
vertliberaux_sales <- merge(sales_data, vertliberaux_data, by.x = "Location")
...
total_sales_df <- data.frame(Party = c("PLR", "PS", "UDC", "Centre", "Verts", "Vertliberaux"),
                             Total_Sales = c(plr_total_sales, ps_total_sales, udc_total_sales,
                                             centre_total_sales, verts_total_sales, vertliberaux_total_sales))
...
distance_matrix <- r.get('distance_matrix')
